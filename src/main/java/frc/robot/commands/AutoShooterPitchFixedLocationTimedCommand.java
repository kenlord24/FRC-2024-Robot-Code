...
public class AutoShooterPitchFixedLocationTimedCommand extends Command 
{
  ...
  final double iLimitDistance = .05;  /*limit ` distance*/
  ...
  public AutoShooterPitchFixedLocationTimedCommand (ShooterPitchSubsystem ShooterPitch, 
                                                    Double Distance,
                                                    Double secondsToRun) 
  {
...
;
    ...
    if (outputSpeed > limitSpeed  )      // limitSpeed = 0.2;
    {
    ...
    if (Math.abs(errorDistance) < .01 )
    {
           ShooterPitch.setShooterPitchSpeed(.01);
    }   
    else
    {
